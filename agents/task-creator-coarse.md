---
name: task-creator-coarse
description: |
  Creates task files from tech-spec Implementation Tasks section (coarse decomposition variant).
  Each task covers one skill's cohesive work scope across the feature — not atomic units.
  Reads actual code files listed in tech-spec, discovers project knowledge,
  generates tasks by task template with checklist-style What to do, TDD Anchor, reviewers, skills.

  Use when: generating task/*.md files after coarse tech-spec is approved,
  during /decompose-tech-spec-coarse or manual task creation from coarse tech-spec.
  Also used in fix mode: receives existing task + validator findings, applies fixes.
  Scope excludes: validating tasks (use task-validator-coarse).
model: inherit
color: green
