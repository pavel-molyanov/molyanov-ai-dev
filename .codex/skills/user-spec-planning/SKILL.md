...
Launch `code-researcher` subagent (spawn_agent, opus) with feature path and feature description from Cycle 1.
...
Launch `interview-completeness-checker` subagent (spawn_agent, sonnet) with feature path. It reviews interview.yml against PK files and code-research.md.
...
Run 2 validators in parallel (spawn_agent):
