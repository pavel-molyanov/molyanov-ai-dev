---
name: claude-agent-task-validator
description: Converted Codex role prompt from Claude agent `task-validator`. Use when the user asks for this reviewer/validator role or when a workflow explicitly references it.
---

# Converted Role: task-validator

Generated from `~/.claude/agents/task-validator.md`.
